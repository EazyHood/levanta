...
- `levanta check` prints how many chunks the network will take the video in and how many seconds of walk each covers. It warns when a chunk covers less than about 20 s (only the ~23 s chunks of the default held their own scale; ~6 s chunks were off by a median 31 %). Past 9 chunks, the longest measured against a real floor, it says what a rendered flat showed up to 14: the scale held within 6 %, but the camera track drifted twice as far and the plan drew the rooms twice.
...
- Fixed a regression that had been sitting in the published TUM example: the room had shrunk from 24.5 to 22.4 m² and its walls from 5.80 to 3.68 m. Round 4 had made an open room's outline snap to the *nearest* parallel wall rather than the one covering most of the edge; measured on seven scenes at once (`bench/planner_sweep.py`), the old scoring is better or equal on every one, so it is back. The 2.5 m reach that arrived in the same change is what actually helped and stays.
- `drop_duplicate_walls`: of two parallel walls closer than 0.5 m running alongside each other, the longer one is drawn (never one whose thickness was measured from both faces). The same surface arrives twice often enough to draw a phantom partition; this removes 2 to 4 walls per benchmark scene and changes no area (`tests/test_duplicate_walls.py`).
- An apartment benchmark, since ARKitScenes has none: `bench/replica.py` walks a real scanned flat (Replica `apartment_0`, 51.8 m², three rooms, two doorways) virtually, renders it off-screen and runs `levanta video` on the result. The verdict: three rooms come out as one or two, no doorway is found, the matched room is 94 % too big, the scale is 1.06-1.07 with no calibration, and the camera track ends 1.0 m off over 35 m. Three attempts at the drift (ceiling as scale anchor, per-chunk TSDF+ICP refinement, chunks composed by overlap) moved nothing and are written up with their numbers.
- `extract_frames` no longer skips whole windows: scoring one frame in three assumed a window of ~30 frames, so a clip already at the target frame rate lost two frames of every three (31 of 116 on the Replica walk). The skip is capped at the window size (`tests/test_frame_sampling_window.py`).
